<?php
echo '<h2>'.$namakriteria.'</h2>';$irdata=array(
	1=>0.00,
	2=>0.00,
	3=>0.58,
	4=>0.90,
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("#formentri").submit(function (e) {
			...
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: "<?=site_url('Perbandingan/updatealt');?>",
				data: $(this).serialize(),
				error: function () {
		...
		$("#form_rata").submit(function (e) {
			e.preventDefault();
			$.ajax({
				type: 'post',
				dataType: 'json',
				url: "<?=site_url('Perbandingan/updaterata');?>",
				data: $(this).serialize(),
				error: function () {
					shownotice('danger', 'Gagal menyimpan data');
					$("#formentri select").removeAttr("disabled");
	...
	function rk() {
		...
		var no1 = 1;
		var ci_r_2 = parseFloat( <?=$jumlah;?> )-no1;
	...
	function coba(v) {
	var target = document.getElementById(v.id).getAttribute("target");
	var ubah = document.getElementById(target);
	// console.log(ubah);
	var nilai = v.value;
			...
			if(nilai == 2){
				...
				ubah.value=0.333333333;
			}else if(nilai == 4){
				ubah.value=0.25;
			}else if(nilai == 5){
				ubah.value=0.2;
			}else if(nilai == 6){
				ubah.value=0.1666666667;
			}else if(nilai == 7){
				ubah.value=0.1428571429;
			}else if(nilai == 8){
				ubah.value=0.125;
			}else if(nilai == 9){
				ubah.value=0.111111111;
			}else{
				ubah.value=1;
			}
...
<div id="entri" class="col-md-16">
	...
	<div class="table-responsive">
		<table class="table table-bordered">
			<thead>
				<th colspan="<?=$jumlah+3;?>" class="text-center">Matrik Perbandingan Alternatif</th>
			</thead>
			<thead>
				<th>Alternatif</th>
						...
						$noSub+=1;
							if($noSub==$noUtama)
							{
								...
								{									
									...
									echo '<td>';
									echo '<select onchange="coba(this)" name="'.$newname.'" id="k'.$noUtama.'b'.$noSub.'" data-target="k'.$noSub.'b'.$noUtama.'" data-kolom="'.$noSub.'" class="form-control inputnumber kolom'.$noSub.'" title="kolom'.$noSub.'">';
									for($x=1;$x<=9;$x++)
									{
										$nilai=ambil_nilai_alternatif($kriteriaid,$k2,$xxx);
										$sl='';
										if($nilai==$x)
										{
			...
			<tfoot>
				<tr>
					...
					<?php
							...
							{
								?>
					<td><input type="text" id="total<?=$h;?>" class="form-control" value="0" title="total<?=$h;?>"
							readonly="" /></td>
					<?php
							}
							?>
...
<div id="matrikdiv" class="col-md-12" style="display: none">
	...
	<div class="table-responsive">
		...
		<table class="table table-bordered">
			<thead>
				<th colspan="<?=$jumlah+5;?>" class="text-center">Matrik Nilai Alternatif</th>
			</thead>
			<thead>
				<th>Alternatif</th>
				<?php
	...
	<div class="table-responsive">
		<table class="table table-bordered">
			<thead>
				<th colspan="<?=$jumlah+1;?>" class="text-center">Perkalian Matrik</th>
			</thead>
			<thead>
				<th>Alternatif</th>
				<?php
	...
	<?php
	echo validation_errors();
	echo form_open('Perbandingan/updaterata',array('class'=>'form-horizontal','id'=>'form_rata2'));
	?>
		<table class="table table-bordered">
			<thead>
			...
			<tbody>
				...
				foreach($arr as $k4=>$v4)
				{
					...
					echo '<td><input type="text" class="form-control" name="id_alternatif['.$k4.']" id="priork-b'.$noUtama4.'" value="0" readonly=""/></td>';
...
?>
<div class="alert alert-danger">Parameter belum dibuat. Silahkan buat terlebih dahulu 
<!-- <a	href="<?=site_url(akses().'/master/kriteria/subkriteria?kriteria='.$kriteriaid);?>">Di sini</a> </div> -->
<?php
	}
?>